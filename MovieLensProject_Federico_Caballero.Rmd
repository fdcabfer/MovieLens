...
```{r edx dimensions, warning=FALSE}
...
```{r final_holdout_test dimensions, warning=FALSE}
...
```{r unique train dataset, warning=FALSE}
...
```{r unique test dataset, warning=FALSE}
...
```{r timestamp lubridate package, warning=FALSE}
...
```{r year timestamp round year, warning=FALSE}
...
```{r title data cleaning, warning=FALSE}
...
Therefore, the rate of a movie has been estimated as following:
  ...
  **pred = mo + m_b + m_y_b + u_b + r_y_b + g_b**
...
```{r prediction test set, warning=FALSE}
...
We can now calculate RMSE using the predictions above and the best lambda (`r best_lambda`):

```{r rmse final, warning=FALSE}
# RMSE with algorithm and best lambda
rmse_final <- RMSE(predicted_values, final_holdout_test$rating)
rmse_final
```

The RMSE is now `r rmse_final`, which shows some significant improvement if we compare it to the first RMSE we obtained, from only the mean of movies.
